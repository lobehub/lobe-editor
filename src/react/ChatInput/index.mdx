...
This team chat scenario connects the mention editor to a small local business layer. Choose one or more recipients with `@`, send a message, then inspect the simulated notification counts. The sender remains “我”; the inspector also exposes the service's explicit `getMentions()` query and the latest committed insertion snapshot. Recipient IDs are deduplicated and filtered by the demo's business handler when the message is sent.
