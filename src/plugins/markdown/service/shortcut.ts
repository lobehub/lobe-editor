...
import { $createRangeSelection, $getSelection, $isLineBreakNode, $isRangeSelection, $isRootOrShadowRoot, $isTextNode, $setSelection, ElementNode, LexicalNode, TextFormatType, TextNode } from "lexical";
import { getOpenTagStartIndex, indexBy, isEqualSubString, PUNCTUATION_OR_SPACE } from "../utils";
...
export class MarkdownShortCutService implements IMarkdownShortCutService {
    ...
    private _textFormatTransformersByTrigger: Readonly<Record<string, ReadonlyArray<TextFormatTransformer>>> | null = null;
    private _textMatchTransformersByTrigger: Readonly<Record<string, Array<TextMatchTransformer>>> | null = null;

    get textMatchTransformersByTrigger() {
        if (!this._textMatchTransformersByTrigger) {
            this._textMatchTransformersByTrigger = indexBy(this.textMatchTransformers, ({ trigger }) => trigger);
        }
    ...
    get textFormatTransformersByTrigger() {
        if (!this._textFormatTransformersByTrigger) {
            this._textFormatTransformersByTrigger = indexBy(this.textFormatTransformers, ({ tag }) => tag.at(-1));
        }
