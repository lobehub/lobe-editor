...
- **Mention Service**: `IMentionService` provides a synchronous document query and editor-scoped insertion subscriptions
...
Use the editor-scoped service for both the current mention snapshot and insertion notifications. `getMentions()` is synchronous, preserves document order and duplicate occurrences, and returns detached metadata snapshots:

```typescript
import { IMentionService } from '@lobehub/editor';

const service = editor.requireService(IMentionService);
const unsubscribe = service?.subscribe((mention) => {
  console.log('mention inserted after commit', mention);
});

const mentions = service?.getMentions() ?? [];
unsubscribe?.();
```

The subscription fires after a successful `INSERT_MENTION_COMMAND` commit. Importing a document, deleting a mention, undoing, or selecting a mention does not emit an insertion notification. Applications can use the callback for UI or analytics, while message delivery should happen from an explicit message submission event. See the complete [ChatInput mention notification scenario](/components/react/chat-input#mention-notifications) for the business layer and local in-memory delivery flow.
