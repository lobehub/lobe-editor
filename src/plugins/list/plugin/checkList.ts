...
function handleCheckItemEvent(event: PointerEvent, callback: () => void) {
  ...
  if (
    target.dir === 'rtl'
      ...
      : clientX > rect.left + beforeMargin - clickAreaPadding &&
        clientX < rect.left + beforeMargin + beforeWidthInPixels + clickAreaPadding
  ) {
    callback();
  }
}
...
function handleArrowUpOrDown(event: KeyboardEvent, editor: LexicalEditor, backward: boolean) {
