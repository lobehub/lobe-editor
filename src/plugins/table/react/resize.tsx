import { $computeTableMapSkipCellCheck, $getTableNodeFromLexicalNodeOrThrow, $getTableRowIndexFromTableCellNode, $isTableCellNode, $isTableRowNode, getDOMCellFromTarget, getTableElement, TableCellNode, TableDOMCell, TableMapType, TableNode } from '@lexical/table';
import { $getNearestNodeFromDOMNode, isHTMLElement, LexicalEditor, NodeKey, SKIP_SCROLL_INTO_VIEW_TAG } from 'lexical';
import { calculateZoomLevel, mergeRegister } from '@lexical/utils';
import React, { CSSProperties, PointerEventHandler, useCallback, useEffect, useMemo, useRef, useState } from 'react';
        ...
        if (activeCell) {
            ...
            if (draggingDirection && pointerCurrentPos && tableRect) {
                if (isHeightChanging(draggingDirection)) {
                    styles[draggingDirection].left = `${window.scrollX + tableRect.left
                        }px`;
                    styles[draggingDirection].top = `${window.scrollY + pointerCurrentPos.y / zoom
                        }px`;
                    styles[draggingDirection].height = '3px';
                    styles[draggingDirection].width = `${tableRect.width}px`;
                } else {
                    styles[draggingDirection].top = `${window.scrollY + tableRect.top}px`;
                    styles[draggingDirection].left = `${window.scrollX + pointerCurrentPos.x / zoom
                        }px`;
