...
const MAX_CONCURRENT_HIGHLIGHTING = 2;
  ...
  editor.update(
    ...
    },
    {
      onUpdate: () => {
        ...
        if (
          nodesCurrentlyHighlighting.size < MAX_CONCURRENT_HIGHLIGHTING &&
          waitingNodesCurrentlyHighlighting.size
        ) {
          const next = waitingNodesCurrentlyHighlighting.values().next().value;
          if (!next) return;
