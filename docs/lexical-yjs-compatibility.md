...
| Lexical command/update scheduling, `COLLABORATION_TAG`, `HISTORIC_TAG`, `onUpdate`, and microtask barriers                        | `src/plugins/yjs/plugin/utils/sync.ts`, `src/plugins/common/node/hole-controller.ts`, `src/plugins/litexml/command/rewriteRange.ts`, `src/headless/collaborative-agent-editor.ts`                                            | `sync-hydration.test.ts`, reconnect tests, Agent lifecycle tests, and focused command/history suites cover ordering and no-echo behavior. The exact microtask sequence remains an implementation assumption and must be rerun after an upgrade.                                                                                                                                                                                                                                                                    |
