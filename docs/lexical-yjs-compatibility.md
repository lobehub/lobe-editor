...
| Rewrite service scheduling and undo grouping                                                                                      | `src/plugins/litexml/command/rewriteRange.ts` and `src/plugins/litexml/__test__/rewrite-range.test.ts`                                                                                                                       | Direct `IRewriteService.rewriteRange` calls participate in the active Lexical update. The legacy `LITEXML_REWRITE_RANGE_COMMAND` adapter first registers the request with the service, then starts it in one next task so the rewrite has its own transaction and default `HISTORY_PUSH_TAG` undo boundary. `history: 'merge'` uses `HISTORY_MERGE_TAG` to join the existing gesture. Completion still comes only from that transaction's per-update `onUpdate` callback.                                                                                                                                                                               |
