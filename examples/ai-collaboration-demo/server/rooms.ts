...
export const getSnapshotUpdate = (roomId: string) => {
  const room = getRoom(roomId);

  return room.hasContent ? encodeStateAsUpdate(room.doc) : null;
};
...
export const clearAssistantAwareness = (roomId: string) => {
  const room = getRoom(roomId);
  let didClear = false;

  for (const [clientID, state] of room.awareness) {
    if (state.name === 'AI Assistant' || state.awarenessData.role === 'assistant') {
      room.awareness.delete(clientID);
      didClear = true;
    }
  }
